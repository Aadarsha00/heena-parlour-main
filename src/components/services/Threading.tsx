...
interface ThreadingServicesProps {
  className?: string;
}
...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, index }) => (
  <Link
    ...
    className="group relative block bg-white/90 backdrop-blur-sm border border-black/5 rounded-3xl p-10 hover:bg-white transition-all duration-700 hover:shadow-2xl hover:shadow-black/10 hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-400/50"
...
const ErrorState: React.FC = () => (
  <section className="w-full bg-gradient-to-b from-amber-50/30 via-white to-amber-50/20 min-h-[60vh] flex items-center justify-center px-6">
...
const EmptyState: React.FC = () => (
  <div className="text-center py-20">
    ...
    <p className="text-gray-500 text-base max-w-md mx-auto leading-relaxed">
      We're carefully curating our premium threading services. Check back soon
      for our exclusive treatments.
    </p>
  </div>
);
...
export default function ThreadingServices({
  className = "",
}: ThreadingServicesProps): JSX.Element {
  const { data, isLoading, error } = useQuery({
    queryKey: ["threadingServices"] as const,
  ...
  return (
    <section
      ...
      className={`relative w-full bg-gradient-to-b from-amber-50/30 via-white to-amber-50/20 py-24 px-6 ${className}`}
    >
      {/* Background Pattern */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-20">
          ...
          <p className="text-gray-600 text-lg md:text-xl font-light max-w-2xl mx-auto leading-relaxed tracking-wide">
            Precision eyebrow artistry using ancient threading techniques for
            flawless results
          </p>
        </div>
      ...
      {/* Bottom Fade */}
      <div className="absolute bottom-0 inset-x-0 h-32 bg-gradient-to-t from-amber-50/30 to-transparent pointer-events-none" />
    </section>
  );
}
